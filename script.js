...
(function () {
  ...
  // ---- Layered Glass / Depth: mouse-parallax on the ambient orb + glass
  // panel (see artDirection.js's 'layered-glass-depth'). Desktop-with-a-
  // precise-pointer only; a no-op everywhere else, including reduced-motion
  // (guarded by the outer `!reducedMotion` on `isMobile`'s sibling check). ----
  if (!reducedMotion && !isMobile && window.matchMedia('(pointer: fine)').matches) {
    var glassOrb = document.querySelector('.glass-depth-orb');
    var glassPanel = document.querySelector('.hero-glass-panel');
    var glassHero = document.querySelector('.hero-glass-depth');
    if (glassHero && (glassOrb || glassPanel)) {
      glassHero.addEventListener('mousemove', function (e) {
        var rect = glassHero.getBoundingClientRect();
        var px = (e.clientX - rect.left) / rect.width - 0.5;
        var py = (e.clientY - rect.top) / rect.height - 0.5;
        if (glassOrb) glassOrb.style.transform = 'translate(calc(-50% + ' + (px * 40) + 'px), calc(-50% + ' + (py * 40) + 'px))';
        if (glassPanel) glassPanel.style.transform = 'translate(' + (px * -12) + 'px, ' + (py * -12) + 'px)';
      });
      glassHero.addEventListener('mouseleave', function () {
        if (glassOrb) glassOrb.style.transform = 'translate(-50%, -50%)';
        if (glassPanel) glassPanel.style.transform = 'translate(0, 0)';
      });
    }
  }

  // ---- Horizontal-Scroll Storytelling: a pinned section whose track
  // translates horizontally as the visitor scrolls vertically past it (see
  // artDirection.js's 'horizontal-scroll-story'). Desktop-only by design —
  // on mobile the CSS already falls back to a plain vertical stack (see
  // base.css's .hscroll-* mobile media query), so this simply never
  // initializes there rather than fighting that fallback. ----
  if (hasScrollTrigger && !isMobile) {
    var hscrollPin = document.querySelector('.hscroll-pin');
    var hscrollTrack = document.querySelector('.hscroll-track');
    if (hscrollPin && hscrollTrack) {
      window.ScrollTrigger.create({
        trigger: hscrollPin, start: 'top top', end: function () { return '+=' + (hscrollTrack.scrollWidth); },
        pin: true, scrub: 1, invalidateOnRefresh: true,
        onUpdate: function (self) {
          var maxTranslate = Math.max(0, hscrollTrack.scrollWidth - window.innerWidth);
          hscrollTrack.style.transform = 'translateX(-' + (self.progress * maxTranslate) + 'px)';
        },
      });
    }
  }
  ...
  if (!reducedMotion) {
    ...
    if (experimentalLayer) {
      ...
      var SCENE_RECIPES = {
        ...
        // Immersive 3D Hero template — a single glowing sphere with sharp
        // dark geometric shard planes orbiting it, evoking a dark
        // product-launch-style composition (see artDirection.js's
        // 'immersive-3d-hero'). Not a literal product — an abstract glow +
        // shard arrangement.
        'orbit-orb': function (THREE, scene, accentColor, isMobile) {
          ...
          orb.position.set(5.6, -0.3, 0); // well right-of-center — keeps the left text column clear (see base.css)
          scene.add(orb);
          var orbLight = new THREE.PointLight(accentColor, 2.2, 20);
          orb.add(orbLight);

          var shardCount = isMobile ? 2 : 4;
          var shards = [];
          for (var i = 0; i < shardCount; i++) {
            var w = 2 + Math.random() * 1.4, h = 3.2 + Math.random() * 1.8;
            var geo = new THREE.PlaneGeometry(w, h);
            var mat = new THREE.MeshStandardMaterial({ color: 0x101014, roughness: 0.35, metalness: 0.4, side: THREE.DoubleSide, transparent: true, opacity: 0.92 });
            var mesh = new THREE.Mesh(geo, mat);
            var angle = (i / shardCount) * Math.PI * 2;
            var radius = 2.4 + Math.random() * 1.2;
            // Clamped so no shard's left edge can drift back toward the
            // text column on the left (see .hero-immersive-3d .hero-inner).
            mesh.position.set(5.6 + Math.max(0, Math.cos(angle)) * radius, (Math.random() - 0.5) * 3, Math.sin(angle) * radius - 2);
          ...
          return {
            ...
            animate: function () {
              ...
              orbMat.emissiveIntensity = 1.0 + Math.sin(clock * 0.8) * 0.25;
              shards.forEach(function (s) {
                s.rotation.z += s.userData.spin;
                s.position.y = s.userData.baseY + Math.sin(clock * 0.5 + s.userData.baseY) * 0.15;
              });
            },
          };
        },
